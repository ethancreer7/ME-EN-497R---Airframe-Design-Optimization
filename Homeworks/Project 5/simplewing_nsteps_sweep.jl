...
results = DataFrame(
nsteps_val = Float64[], CL = Float64[], CD = Float64[])
...
mkdir(joinpath(@__DIR__, "wings_nsteps"))

for (count, i) in enumerate(range(10, 150, step = 10))
  run_name        = "wing_nsteps$(i)"            # Name of this simulation

  save_path       = joinpath(@__DIR__, "wings_nsteps", run_name)        # Where to save this simulation
  ...
  path = joinpath(@__DIR__, "wings_nsteps", run_name, run_name * "_convergence.csv")                    
